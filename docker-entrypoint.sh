...
if [ -n "${HTTP_PORT}" ]; then
  echo "Replacing default HTTP port (1180) with the value specified by the user - (HTTPS_PORT: ${HTTP_PORT})."
  sed -i "s/1180/${HTTP_PORT}/g"  /etc/nginx/nginx.conf
fi

if [ -n "${HTTPS_PORT}" ]; then
  echo "Replacing default HTTPS port (11443) with the value specified by the user - (HTTPS_PORT: ${HTTPS_PORT})."
